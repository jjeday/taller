...
class ListUs extends TPage
{
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new TQuickForm('form_search_BlogUs');
        ...
        $this->form->setData(TSession::getValue('BlogUs_filter_data'));

        // add the search form actions
        $this->form->addQuickAction(_t('Find'), new TAction(array($this, 'onSearch')), 'fa:search');
        $this->form->addQuickAction(_t('New'), new TAction(array('FormUs', 'onEdit')), 'bs:plus-sign green');
        ...
        $column_titulo = new TDataGridColumn('description', 'Descripción', 'left');
        ...
        // create EDIT action
        $action_edit = new TDataGridAction(array('FormUs', 'onEdit'));
    ...
    public function onInlineEdit($param)
    {
        try
        {
            ...
            $object = new BlogUs($key); // instantiates the Active Record
            $object->{$field} = $value;
    ...
    public function onSearch()
    {
        ...
        $filters = array();
        TSession::setValue('BlogUs_filters', array());
        TSession::setValue('BlogUs_id', '');
        TSession::setValue('BlogUs_name', '');
        TSession::setValue('BlogUs_tipo', '');
        ...
        if ($data->description)
        {
            ...
            // stores the filter in the session
            TSession::setValue('BlogUs_name', $data->description);
            $filters[] = $filter;
        }
        ...
        TSession::setValue('BlogUs_filter_data', $data);

        // keep the search data in the session
        TSession::setValue('BlogUs_filters_data', $filters);
    ...
    public function onReload($param = null)
    {
        try
        {
            ...
            $repository = new TRepository('BlogUs');
            $limit = 10;
            // creates a criteria
            ...
            if (TSession::getValue('BlogUs_filters_data'))
            {
                foreach (TSession::getValue('BlogUs_filters_data') as $filter)
                {
    ...
    public function Delete($param)
    {
        try
        {
            ...
            TTransaction::open('database'); // open a transaction with database
            $object = new BlogUs($key, false); // instantiates the Active Record
