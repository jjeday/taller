...
class WebAlertMessageList extends TElement
{
    public function __construct($param)
    {
        ...
        try
        {
            ...
            $system_messages = WebMessage::where('checked', '=', 'N')->where('mensaje_tipo', '=', 'Recibido')->orderBy('id', 'desc')->load();
            ...
            if ($param['theme'] == 'theme2')
            {
                ...
                foreach ($system_messages as $system_message)
                {
                    ...
                    $a->href = 'index.php?class=WebMessageFormView&method=onView&id=' . $system_message->id;
                    $a->generator = 'adianti';
                    $li->add($a);
                    $a->add($div);
                ...
                $li->add($a);
                $a->class = 'text-center';
                $a->href = 'index.php?class=WebMessageList&method=filterInbox';
                $a->generator = 'adianti';
                ...
                $li->add($a);
                $a->class = 'text-center';
                $a->href = 'index.php?class=WebMessageForm';
                $a->generator = 'adianti';
                $a->add(TElement::tag('strong', 'Send message'));
                $a->add($i = TElement::tag('i', '', array('class' => 'fa fa-user-circle')));
                parent::add($li);
            } else
                if ($param['theme'] = 'theme3')
                {
                    ...
                    $a->add(TElement::tag('i', '', array('class' => "fa fa-commenting-o fa-fw")));
                    if (count($system_messages) > 0)
                    {
                    ...
                    parent::add(TElement::tag('li', TElement::tag('a', 'Read messages', array('href' => 'index.php?class=WebMessageList&method=filterInbox', 'generator' => 'adianti')), array('class' => 'footer')));
                    parent::add(TElement::tag('li', TElement::tag('a', 'Send messages', array('href' => 'index.php?class=WebMessageForm', 'generator' => 'adianti')), array('class' => 'footer')));
                }
