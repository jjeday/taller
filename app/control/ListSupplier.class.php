 ...
 */
class ListSupplier extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new TQuickForm('form_search_BlogSupplier');
        $this->form->class = 'tform'; // change CSS class
        $this->form = new BootstrapFormWrapper(new TQuickForm);
        $this->form->style = 'display: table;width:100%'; // change style
        $this->form->setFormTitle('Proveedores');
        ...
        $this->form->setData(TSession::getValue('BlogSupplier_filter_data'));

        // add the search form actions
        $this->form->addQuickAction(_t('Find'), new TAction(array($this, 'onSearch')), 'fa:search');
        $this->form->addQuickAction(_t('New'), new TAction(array('FormSupplier', 'onEdit')), 'bs:plus-sign green');
        ...
        $column_titulo = new TDataGridColumn('name', 'Nombre', 'left');
        ...
        // create EDIT action
        $action_edit = new TDataGridAction(array('FormSupplier', 'onEdit'));
        ...
        $panel = new TPanelGroup('Proveedores');
        $panel->add($this->form);
    ...
    public function onInlineEdit($param)
    {
        try
        {
            ...
            $object = new BlogSupplier($key); // instantiates the Active Record
            $object->{$field} = $value;
    ...
    public function onSearch()
    {
        ...
        $filters = array();
        TSession::setValue('BlogSupplier_filters', array());
        TSession::setValue('BlogSupplier_id', '');
        TSession::setValue('BlogSupplier_name', '');
        TSession::setValue('BlogSupplier_tipo', '');
        ...
        /*if ($data->id)
        {
            ...
            TSession::setValue('BlogClient_categoria_id', $data->id);
            $filters[] = $filter;
        }*/
        ...
        if ($data->name)
        {
            ...
            // stores the filter in the session
            TSession::setValue('BlogSupplier_name', $data->name);
            $filters[] = $filter;
        }
        ...
        TSession::setValue('BlogClient_filter_data', $data);

        // keep the search data in the session
        TSession::setValue('BlogClient_filters_data', $filters);
    ...
    public function onReload($param = null)
    {
        try
        {
            ...
            $repository = new TRepository('BlogSupplier');
            $limit = 10;
            // creates a criteria
            ...
            if (TSession::getValue('BlogSupplier_filters_data'))
            {
                foreach (TSession::getValue('BlogSupplier_filters_data') as $filter)
                {
    ...
    public function Delete($param)
    {
        try
        {
            ...
            TTransaction::open('database'); // open a transaction with database
            $object = new BlogSupplier($key, false); // instantiates the Active Record
