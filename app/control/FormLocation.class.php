 ...
 */
class FormLocation extends TPage
{
    protected $form; // form
    ...
    public function __construct($param)
    {
        ...
        // creates the form
        $this->form = new TQuickForm('form_BlogLocation');
        $this->form->class = 'tform'; // change CSS class
        $this->form = new BootstrapFormBuilder('form_BlogLocation');
        $this->form->style = 'display: table;width:100%'; // change style
        //$this->form->setFieldsByRow(1);

        // define the form title
        $this->form->setFormTitle('Ubicación');
        ...
        $phone->setSize('100%');
        $email->setSize('100%');
        $schedule->setSize('100%');
        
        $phone->setTip('Teléfono: ex. +51 XXX XXX XXX');


        
        //$categoria_id->addValidation('Categoria Id', new TRequiredValidator);
        $address->addValidation('Dirección', new TRequiredValidator);
        $phone->addValidation('Teléfono', new TRequiredValidator);
        ...
        $this->form->addFields([new TLabel('Dirección')], [$address]);
        $this->form->addFields([new TLabel('Teléfono')], [$phone]);
        $this->form->addFields([new TLabel('Email')], [$email]);
        $this->form->addFields([new TLabel('Horario')], [$schedule]);
        ...
        $list = $this->form->addAction(_t('List'), new TAction(array('ListLocation', 'onLoad')), 'bs:list');
    ...
    public function onSave($param)
    {
        try
        {
            ...
            $object = (empty($data->id)) ? new BlogLocation : new BlogLocation($data->id, true); // create an empty object
