...
class FormCategory extends TPage
{
    ...
    public function __construct($param)
    {
        ...
        // creates the form
        $this->form = new TQuickForm('form_BlogCategory');
        $this->form->class = 'tform'; // change CSS class
        $this->form = new BootstrapFormBuilder('form_BlogCategory');
        $this->form->style = 'display: table;width:100%'; // change style
        //$this->form->setFieldsByRow(1);

        // define the form title
        $this->form->setFormTitle('Categorías');
        ...
        $name->setTip('Nombre de la categoría');
       
        //$id->addValidation('Categoría ID', new TRequiredValidator);
        ...
        $this->form->addFields([new TLabel('Categoría')], [$name]);
        ...
        $list = $this->form->addAction(_t('List'), new TAction(array('ListCategory', 'onLoad')), 'bs:list');
    ...
    public function onSave($param)
    {
        try
        {
            ...
            $object = (empty($data->id)) ? new BlogCategory : new BlogCategory($data->id, true); // create an empty object
            ...
            if ($data->image != '')
            {
                ...
                if (file_exists($source_file))
                {
                    ...
                    if ($object->image_head != TSession::getValue('image'))
                    {
    ...
    public function onEdit($param)
    {
        try
        {
            if (isset($param['key']))
            {
                ...
                TTransaction::open('database'); // open a transaction
                $object = new BlogCategory($key); // instantiates the Active Record
