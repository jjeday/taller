 ...
 */
class ListClient extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new TQuickForm('form_search_BlogClient');
        ...
        $this->form->addQuickField('Empresa', $company, '100%' /*, new TRequiredValidator */ );


        // keep the form filled during navigation with session data
        $this->form->setData(TSession::getValue('BlogClient_filter_data'));

        // add the search form actions
        $this->form->addQuickAction(_t('Find'), new TAction(array($this, 'onSearch')), 'fa:search');
        $this->form->addQuickAction(_t('New'), new TAction(array('FormClient', 'onEdit')), 'bs:plus-sign green');
        ...
        $column_tipo = new TDataGridColumn('company', 'Empresa', 'left');
        $this->datagrid->addColumn($column_tipo);
        $column_etiqueta = new TDataGridColumn('comment', 'Comentario', 'left');
        ...
        // create EDIT action
        $action_edit = new TDataGridAction(array('FormClient', 'onEdit'));
    ...
    public function onSearch()
    {
        ...
        $filters = array();
        TSession::setValue('BlogClient_filters', array());
        TSession::setValue('BlogClient_id', '');
        TSession::setValue('BlogClient_name', '');
        TSession::setValue('BlogClient_tipo', '');
        ...
        if ($data->name)
        {
            ...
            // stores the filter in the session
            TSession::setValue('BlogClient_name', $data->name);
            $filters[] = $filter;
        }
    ...
    public function onReload($param = null)
    {
        try
        {
            ...
            $repository = new TRepository('BlogClient');
            $limit = 10;
            // creates a criteria
            ...
            if (TSession::getValue('BlogClient_filters_data'))
            {
                foreach (TSession::getValue('BlogClient_filters_data') as $filter)
                {
